...
class Warehouse:
    ...
    def _add_inventory(self):
        ...
        weigths = [random.randrange(0, 100) / 10 for _ in locations]
        volumes = [random.randrange(0, 100) / 100 for _ in locations]
        inventory = []
    ...
    def step(self):
        ...
        if self.next_action == "wait":
            ...
            if self.model.orders and self.model.available_amr:
                ...
                #if the path has movements to do, keep tellinh him to move, as the number of positions in the path
                if self.path:
...
o_warehousemodel = WarehouseModel(warehouse=o_warehouse, n_pickers=2, n_amr=2, orders=o_orders)
